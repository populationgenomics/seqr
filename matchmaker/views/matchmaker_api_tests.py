...
SUBMISSION_DATA = {
    ...
    "contactHref": "mailto:test@populationgenomics.org.au",
    "contactName": "PI",
    "phenotypes": [
...
NEW_MATCH_JSON = {
    "score": {"patient": 0.92},
    "patient": {
        "genomicFeatures": [{"gene": {"id": "OR4F29"}}],
        "features": [
            {"observed": "yes", "id": "HP:0012469", "label": "Infantile spasms"}
        ],
        "contact": {
...
MISMATCHED_GENE_NEW_MATCH_JSON["patient"]["genomicFeatures"][0]["gene"][
    "id"
] = "ENSG00000227232"
...
class MatchmakerAPITest(AuthenticationTestCase):
    ...
    @mock.patch(
        "seqr.views.utils.permissions_utils.ANALYST_PROJECT_CATEGORY",
        "analyst-projects",
    )
    ...
    def test_get_individual_mme_matches(self, mock_analyst_group):
        ...
        invalid_url = reverse(
            get_individual_mme_matches, args=[INVALID_PROJECT_SUBMISSION_GUID]
        )
        ...
        self.assertDictEqual(
            response_json["mmeResultsByGuid"][RESULT_STATUS_GUID], PARSED_RESULT
        )
        self.assertFalse(
            "originatingSubmission"
            in response_json["mmeResultsByGuid"]["MR0007228_VCGS_FAM50_156"]
        )
        self.assertDictEqual(
            response_json["mmeSubmissionsByGuid"],
            {
                SUBMISSION_GUID: {
                    ...
                    "contactHref": "mailto:matchmaker@populationgenomics.org.au,test_user@populationgenomics.org.au",
        ...
        self.assertDictEqual(
            response_json["individualsByGuid"],
            {
                INDIVIDUAL_GUID: {
        ...
        self.assertSetEqual(
            set(
                response_json["mmeSubmissionsByGuid"][SUBMISSION_GUID]["mmeResultGuids"]
            ),
            {"MR0007228_VCGS_FAM50_156", "MR0004688_RGP_105_3", RESULT_STATUS_GUID},
        )
        ...
        self.assertSetEqual(
            set(response_json["savedVariantsByGuid"].keys()),
            {
                ...
                "SV0059957_11562437_f019313_1",
                "SV0059956_11560662_f019313_1",
            },
        )
        ...
        self.assertDictEqual(
            response_json["mmeResultsByGuid"]["MR0007228_VCGS_FAM50_156"][
                "originatingSubmission"
            ],
            {
    ...
    def test_search_individual_mme_matches(
        self, mock_nodes, mock_email, mock_slacker, mock_logger
    ):
        mock_slacker.return_value.chat.post_message.side_effect = ValueError(
        ...
        responses.add(
            responses.POST, "http://node_a.com/match", body="Failed request", status=400
        )
        responses.add(
            responses.POST,
            "http://node_b.mme.org/api",
            status=200,
            json={
                "results": [
                    NEW_MATCH_JSON,
                    INVALID_NEW_MATCH_JSON,
                    MISMATCHED_GENE_NEW_MATCH_JSON,
                ]
            },
        )
        ...
        new_result_guid = next(
            k
            for k in response_json["mmeResultsByGuid"].keys()
            if k not in {"MR0004688_RGP_105_3", RESULT_STATUS_GUID}
        )

        self.assertDictEqual(
            response_json["mmeResultsByGuid"][new_result_guid], PARSED_NEW_MATCH_JSON
        )
        self.assertTrue(
            response_json["mmeResultsByGuid"]["MR0004688_RGP_105_3"]["matchStatus"][
                "matchRemoved"
            ]
        )
        self.assertFalse(
            response_json["mmeResultsByGuid"][RESULT_STATUS_GUID]["matchStatus"][
                "matchRemoved"
            ]
        )
        self.assertDictEqual(
        ...
        self.assertDictEqual(
            response_json["mmeContactNotes"], {"st georges, university of london": {}}
        )

        #  Test removed match is deleted
        self.assertEqual(
            MatchmakerResult.objects.filter(guid="MR0007228_VCGS_FAM50_156").count(), 0
        )
        ...
        expected_body = json.dumps(
            dict(_disclaimer=MME_DISCLAIMER, **expected_patient_body)
        )
    ...
    @mock.patch(
        "matchmaker.views.matchmaker_api.EmailMessage", Exception("Email error")
    )
    @mock.patch("matchmaker.views.matchmaker_api.MME_NODES")
    @responses.activate
    def test_update_mme_submission(self, mock_mme_nodes):
        responses.add(
            responses.POST,
            "http://node_a.com/match",
            status=200,
            json={"results": [NEW_MATCH_JSON]},
        )
        mock_mme_nodes.values.return_value = [
            {"name": "Node A", "token": "abc", "url": "http://node_a.com/match"}
        ]
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.reason_phrase, "Gene id is required for genomic features"
        )

        response = self.client.post(
            url,
            content_type="application/json",
            data=json.dumps({"phenotypes": [{"id": "HP:0012469"}]}),
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.reason_phrase, "Individual is required for a new submission"
        )

        # Test successful creation
        response = self.client.post(
            url, content_type="application/json", data=json.dumps(SUBMISSION_DATA)
        )
        self.assertEqual(response.status_code, 200)
        response_json = response.json()
        self.assertSetEqual(
        ...
        self.assertEqual(
            response_json["mmeSubmissionsByGuid"][new_submission_guid][
                "lastModifiedDate"
            ][:10],
            datetime.today().strftime("%Y-%m-%d"),
        )
        ...
        self.assertDictEqual(
            response_json["mmeResultsByGuid"][new_match_result_guid],
            PARSED_NEW_MATCH_NEW_SUBMISSION_JSON,
        )
        self.assertEqual(
            next(iter(response_json["mmeResultsByGuid"].values()))["submissionGuid"],
            new_submission_guid,
        )
        self.assertSetEqual(
            set(response_json["genesById"].keys()),
            {"ENSG00000186092", "ENSG00000235249"},
        )
        self.assertListEqual(
            list(response_json["mmeContactNotes"].keys()),
            ["st georges, university of london"],
        )
        ...
        expected_body = {
            "patient": {
                ...
                "label": "HG00733",
                "contact": {
                    "href": "mailto:test@populationgenomics.org.au",
                    "name": "PI",
                    "institution": "Broad Center for Mendelian Genomics",
                },
                "sex": "FEMALE",
        ...
        response = self.client.post(
            url, content_type="application/json", data=json.dumps(update_body)
        )
        ...
        new_internal_match_guid = next(
            k
            for k in response_json["mmeResultsByGuid"].keys()
            if k != new_match_result_guid
        )
        self.assertDictEqual(
            response_json["mmeResultsByGuid"][new_internal_match_guid],
            {
                "id": "NA20885",
                "score": 0,
                "patient": {
        ...
        self.assertDictEqual(
            ...
            {
                new_submission_guid: {
                    "mmeResultGuids": sorted(
                        list(response_json["mmeResultsByGuid"].keys()), reverse=True
                    ),
                    "individualGuid": NO_SUBMISSION_INDIVIDUAL_GUID,
                    "submissionGuid": new_submission_guid,
                    "createdDate": mock.ANY,
        ...
        self.assertSetEqual(
            set(response_json["genesById"].keys()),
            {"ENSG00000186092", "ENSG00000235249", "ENSG00000227232"},
        )
        self.assertListEqual(
            list(response_json["mmeContactNotes"].keys()),
            ["st georges, university of london"],
        ...
        self.assertEqual(
            responses.calls[1].request.headers["Accept"],
            "application/vnd.ga4gh.matchmaker.v1.0+json",
        )
        self.assertEqual(
            responses.calls[1].request.headers["Content-Type"],
            "application/vnd.ga4gh.matchmaker.v1.0+json",
        )
        ...
        matched_result = MatchmakerResult.objects.get(
            originating_submission__guid=new_submission_guid
        )
    ...
    def test_delete_mme_submission(self, mock_mme_nodes):
        ...
        self.assertEqual(
            MatchmakerResult.objects.filter(submission__guid=SUBMISSION_GUID).count(), 2
        )
        ...
        responses.add(
            responses.POST, "http://node_a.com/match", status=200, json={"results": []}
        )
        mock_mme_nodes.values.return_value = [
            {"name": "Node A", "token": "abc", "url": "http://node_a.com/match"}
        ]
        ...
        self.assertEqual(
            response_json["mmeSubmissionsByGuid"][SUBMISSION_GUID]["lastModifiedDate"][
                :10
            ],
            datetime.today().strftime("%Y-%m-%d"),
        )
