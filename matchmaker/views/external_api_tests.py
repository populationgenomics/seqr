...
class ExternalAPITest(TestCase):
    ...
    def _check_mme_authenticated(self, url):
        ...
        response = self.client.get(
            url, HTTP_ACCEPT="application/vnd.ga4gh.matchmaker.v1.0+json"
        )
        self.assertEqual(response.status_code, 401)

        response = self.client.get(
            url,
            HTTP_ACCEPT="application/vnd.ga4gh.matchmaker.v1.0+json",
    ...
    def _make_mme_request(self, url, method, **kwargs):
        ...
        return call_func(
            url,
            HTTP_ACCEPT="application/vnd.ga4gh.matchmaker.v1.0+json",
            HTTP_X_AUTH_TOKEN=TEST_ACCESS_TOKEN,
            **kwargs
        )
    ...
    def test_mme_match_proxy(self, mock_post_to_slack, mock_email):
        ...
        response = self._make_mme_request(
            url, "post", content_type="application/json", data="Invalid body"
        )
        self.assertEqual(response.status_code, 400)
        response_json = response.json()
        self.assertListEqual(list(response_json.keys()), ["error"])
        ...
        response = self._make_mme_request(
            url, "post", content_type="application/json", data=json.dumps({})
        )
        self.assertEqual(response.status_code, 400)
        ...
        response = self._make_mme_request(
            ...
            "post",
            content_type="application/json",
            data=json.dumps({"patient": {"id": 123}}),
        )
        self.assertEqual(response.status_code, 400)
        ...
        self.assertEqual(response.status_code, 400)
        self.assertDictEqual(
            response.json(), {"error": '"features" or "genomicFeatures" are required'}
        )

        # Test valid request
        response = self._make_mme_request(
            url, "post", content_type="application/json", data=json.dumps(request_body)
        )
        self.assertEqual(response.status_code, 200)
            ...
            results[1],
            {
                "patient": {
                    ...
                    "genomicFeatures": [
                        {
                            "gene": {"id": "ENSG00000186092"},
                            "variant": {"referenceName": "14", "start": 77027630},
                        },
                        {"gene": {"id": "ENSG00000233750"}},
                        {"gene": {"id": "ENSG00000223972"}},
                        {"gene": {"id": "ABC"}},
                    ],
                },
                "score": {
        ...
        self.assertEqual(
            MatchmakerIncomingQuery.objects.filter(patient_id="12345").count(), 1
        )
...
be found found at https://seqr.broadinstitute.org/matchmaker/disclaimer."""
        ...
        mock_post_to_slack.assert_called_with(
            ...
            message_template.format(
                ...
                emails="UDNCC@hms.harvard.edu, matchmaker@phenomecentral.org, test_user@populationgenomics.org.au",
            ),
        )
        ...
        mock_email.assert_has_calls(
            [
                ...
                mock.call(
                    ...
                    body=message_template.format(
                        matches=match2,
                        emails="UDNCC@hms.harvard.edu, matchmaker@phenomecentral.org",
                    ),
        ...
        self.assertEqual(
            MatchmakerIncomingQuery.objects.filter(patient_id="12345").count(), 2
        )
        mock_post_to_slack.assert_called_with(
            "matchmaker_matches",
    ...
    def test_mme_match_proxy_no_results(self, mock_post_to_slack, mock_email):
        ...
        incoming_query_q = MatchmakerIncomingQuery.objects.filter(
            institution="Test Institute"
        )
        ...
        self.assertEqual(
            MatchmakerIncomingQuery.objects.filter(
                institution="Test Institute"
            ).count(),
            2,
        )
