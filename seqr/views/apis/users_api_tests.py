...
class UsersAPITest(object):
    ...
    """.format(
        ...
        mock_logger.info.assert_called_with(
            "Created user test@test.com (local)", extra={"user": self.manager_user}
        )
    ...
    def test_set_password(self):
        ...
        response = self.client.post(
            set_password_url, content_type="application/json", data=json.dumps({})
        )
        self.assertEqual(response.status_code, 403)
        ...
        response = self.client.post(
            set_password_url,
            content_type="application/json",
            data=json.dumps({"userToken": "invalid"}),
        )
        self.assertEqual(response.status_code, 403)
        self.assertEqual(response.json()["error"], "Not authorized to update password")
        ...
        response = self.client.post(
            set_password_url,
            content_type="application/json",
            data=json.dumps({"userToken": quote_plus(password)}),
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_forgot_password(self, mock_send_mail):
        ...
        response = self.client.post(
            url,
            content_type="application/json",
            data=json.dumps({"email": "test_new_user@test.com"}),
        )
        self.assertEqual(response.status_code, 400)
