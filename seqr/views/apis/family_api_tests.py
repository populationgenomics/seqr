...
class FamilyAPITest(AuthenticationTestCase):
    ...
    def test_edit_families_handler(self, mock_pm_group):
        ...
        req_values = {
            "families": [
                ...
                {
                    PREVIOUS_FAMILY_ID_FIELD: "2",
                    FAMILY_ID_FIELD: "22",
                    "description": "Test description 2",
                },
                {
                    FAMILY_ID_FIELD: "new_family",
                    "description": "Test descriptions for a new family",
                },
            ]
        }
        response = self.client.post(
            url, content_type="application/json", data=json.dumps(req_values)
        )
        ...
        self.assertEqual(
            response_json["familiesByGuid"][FAMILY_GUID]["description"],
            "Test description 1",
        )
        self.assertEqual(
            response_json["familiesByGuid"]["F000002_2"][FAMILY_ID_FIELD], "22"
        )
        self.assertEqual(
            response_json["familiesByGuid"]["F000002_2"]["description"],
            "Test description 2",
        )
        new_guids = set(response_json["familiesByGuid"].keys()) - set(
            [FAMILY_GUID, "F000002_2"]
        )
        new_guid = new_guids.pop()
        self.assertEqual(
            response_json["familiesByGuid"][new_guid]["description"],
            "Test descriptions for a new family",
        )
        ...
        response = self.client.post(
            ...
            data=json.dumps({"families": [{"familyGuid": "F000012_12"}]}),
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_families_handler(self, mock_pm_group):
        ...
        req_values = {
            "families": [{"familyGuid": FAMILY_GUID}, {"familyGuid": FAMILY_GUID2}]
        }
        response = self.client.post(
            url, content_type="application/json", data=json.dumps(req_values)
        )
        ...
        response_json = response.json()
        self.assertSetEqual(
            set(response_json.keys()), {"individualsByGuid", "familiesByGuid"}
        )
        self.assertIsNone(response_json["familiesByGuid"][FAMILY_GUID])
        self.assertIsNone(response_json["familiesByGuid"][FAMILY_GUID2])

        response = self.client.post(
            url, content_type="application/json", data=json.dumps({"families": None})
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_update_family_analysed_by(self):
        ...
        self.assertEqual(
            response_json[FAMILY_GUID]["analysedBy"][0]["createdBy"]["fullName"],
    ...
    def test_update_family_pedigree_image(self):
        ...
        self.assertListEqual(list(response_json.keys()), [FAMILY_GUID])
        self.assertRegex(
            response_json[FAMILY_GUID]["pedigreeImage"],
    ...
    def test_update_success_story_types(self, mock_analyst_group):
        ...
        self.assertListEqual(
            response_json[FAMILY_GUID]["successStoryTypes"], ["O", "D"]
        )
    ...
    def test_receive_families_table_handler(self, mock_pm_group):
        ...
        response = self.client.post(
            url, {"f": SimpleUploadedFile("1000_genomes demo_families.tsv", data)}
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.reason_phrase, "Invalid header, missing family id column"
        )
        ...
        self.assertDictEqual(
            ...
            {
                "errors": [
                    "Could not find families with the following previous IDs: 1_old"
                ],
                "warnings": [],
            },
        )
...
"2"	""	"2"	"family two description"	""'
        ...
        self.assertSetEqual(
            set(response_json.keys()), {"info", "errors", "warnings", "uploadedFileId"}
        )
        ...
        self.assertListEqual(list(response_json.keys()), ["familiesByGuid"])
        self.assertSetEqual(
            set(response_json["familiesByGuid"].keys()), {FAMILY_GUID2, FAMILY_GUID}
        )
        ...
        response = self.client.post(
            url,
            {"f": SimpleUploadedFile("families.tsv", "Family ID\n1".encode("utf-8"))},
        )
        self.assertEqual(response.status_code, 200)
