...
class TransferFamiliesTest(TestCase):
    ...
    def test_command(self, mock_logger):
        ...
        self.assertEqual(
            str(err.exception), "Error: the following arguments are required: email"
        )
        ...
        self.assertEqual(
            MatchmakerSubmission.objects.get(id=3).contact_href,
            "mailto:matchmaker@phenomecentral.org",
        )
        self.assertEqual(
            MatchmakerSubmission.objects.get(id=1).contact_href,
            "mailto:matchmaker@populationgenomics.org.au",
        )
